    ...
    <DashboardLayout userType="business">
      <div className="max-w-[1100px] mx-auto space-y-10">
        <section className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <Link to="/topup" className="flex-1 md:flex-none">
              <Button className="w-full gap-2 h-14 shadow-2xl shadow-primary/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <Card className="p-8 bg-primary-container text-white adire-texture relative overflow-hidden border-0 shadow-xl">
             <div className="relative z-10">
               <p className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-60 mb-4">Infrastructure Balance</p>
               <h2 className="font-display font-extrabold text-5xl mb-8">₦840,200</h2>
               <div className="flex items-center gap-4 text-xs font-bold uppercase tracking-widest bg-white/10 w-fit px-3 py-1 rounded-full">
                  <Zap size={14} className="animate-pulse" /> Instant Node Active
               </div>
             </div>
          </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-10">
           <section className="space-y-6">
              <h2 className="font-display font-bold text-2xl px-2">Financial Audit Log</h2>
                 ...
                 {transactions.map((tx) => (
                   <div key={tx.id} className="flex items-center justify-between p-6 bg-surface-low rounded-2xl hover:bg-surface-container transition-all">
                      <div className="flex items-center gap-4">
                        <div className={cn(
                          "p-3 rounded-xl",
                          tx.amount.startsWith('+') ? "bg-emerald-100 text-emerald-600" : "bg-secondary/10 text-secondary"
                        )}>
                      ...
                      <div className="text-right">
                         ...
                         <p className="text-[9px] font-bold text-emerald-500 uppercase tracking-widest">{tx.status}</p>
                      </div>
                   </div>
                 ))}
              </div>
           ...
           <aside className="space-y-8">
              <Card className="p-8 bg-surface-high border border-outline-variant/10 relative overflow-hidden">
                 ...
                 <p className="text-[8px] font-bold text-on-surface/30 uppercase tracking-widest mt-4">AGRILINK NIGERIA LTD</p>
              </Card>
           </aside>
        </div>
      </div>
